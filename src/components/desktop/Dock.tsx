...
export function Dock() {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-6 z-[9999] flex justify-center">
      <div
        className="desktop-shadow pointer-events-auto flex items-end gap-3 rounded-[1.7rem] border border-white/20 bg-white/12 px-5 pb-3 pt-3 backdrop-blur-2xl"
      >
        {dockApps.map((app) => {
          ...
          return (
            <div key={`${app.id}-${app.label}`} className="group relative flex cursor-pointer flex-col items-center">
              ...
              <div className={`absolute -bottom-1.5 h-1 w-1 rounded-full bg-white/85 transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0'}`} />
            </div>
          );
        })}
      </div>
